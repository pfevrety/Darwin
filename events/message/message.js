...
// maintenance mode
            ...
            if (customCommandSettings && customCommandSettings.name && !customCommandSettings.description && customCommandSettings.json == "true") {
                ...
                return message.channel.send(command).catch((e) => {
                    message.channel.send(`There was a problem sending your embed, which is probably a JSON error.\nRead more here --> https://pogy.xyz/embeds\n\n__Error:__\n\`${e}\``)
                })
            }
            ...
            if (command) {
                ...
                if (!command.voted) {
                    let voted = true;
                    if (!voted) {
                        return message.channel.send("You should vote to execute this command: ``.vote``")
                    }
                }
